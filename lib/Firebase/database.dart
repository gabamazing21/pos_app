  ...
  static Future<List<Menu>> getMenu() async {
    ...
    menuRef().snapshots().listen((event) {
      event.docs.forEach((element) {
        ...
        Menu item = new Menu(element['menuName'],(element["subMenu"] != null?List.from(element['subMenu']):null), element['imageLink'],
            element['visibility']);
...
static Future<List<submenu>> getAllSubMenu(){
  ...
  subMenuRef().snapshots().listen((event) {
    ...
    items.add(new submenu(value['title'], value.id, value['imageUrl'], 0.0,
        List.from(value['modifiers']), (value["subMenu"] != null)?List.from(value['subMenu']):null));
...
static Future<List<Item>> getItems() async{
  List<Item> itemList=List();
    ItemRef().snapshots().listen((event) {
      ...
      Item  item = new Item(
            element['itemName'], element.id, element['ItemPrice'], element['visible']);
      itemList.add(item);
      });
  ...
  static Future<submenu> getsubmenu(String id) async {
    ...
    new submenu(value['title'], value.id, value['imageUrl'], 0.0,
        List.from(value['modifiers']), (value["subMenu"] != null)?List.from(value['subMenu']):null));
    return item;
  }
  ...
  static Future<List<modifiers>> getAllModifier(){
    ...
    modifierRef().snapshots().listen((event) {
      event.docs.forEach((element) {
        ...
        modifiers item =
        new modifiers(element.id,element['modifiersName'], List.from(element["items"]), 0,
            element['checkBox']);
        items.add(item);
